...
- 비관리 리소스는 Finalizer 와 IDisposable인터페이스라는 두가지 메커니즘 제공하지만, Finalizer를 포함하고 있는 객체는 Finalizer를 호출하기 위해 종료 큐에 임시 저장되어 바로 가비지 컬렉트 되지 않는다.
- 또한 언제 Finalize 될지 알 수도 없다. 그래서…
- 비관리 리소스를 안전하게 해제하는 좋은 방법. (책애서 소개된 것과 다른 예제)
```
