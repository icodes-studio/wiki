...
- ***채팅 메시지를 저장하는 테이블 생성***
    ...
    -  이름으로 테이블을 만들고, room_id(문자) 를 hashkey로, timestamp(숫자)를 Rangekey로 설정합니다.

- ***유저 목록을 저장하는 테이블 생성***
    - chatapp-userlist 이름으로 테이블을 만들고, connection_id(문자)를 hashkey로 설정합니다. 그리고 room_id(문자) Hashkey, user_id(문자) range key로 구성된 글로벌 보조 인덱스(Global Secondary Index)를 설정합니다.
