...
4.5.2.4 Indexer
배열의 요소에 접근할 때 처럼 일반 클래스의 요소에 접근하려면 this 예약어를 이용한 인덱서(indexer)라고 하는 특별한 구문을 이용해야 한다.
...
특성 클래스를 인스턴스화할 수 있는 독특한 구문이 존재하는데, 바로 대괄호다.
...
AttributeTargets에 정의된 값을 보면 특성이 적용될 수 있는 대상을 확인할 수 있다.
...
메서드 콜스택이 많이 쌓여서 1MB 용량을 넘어가는 상황이 생길 수 있는데 이를 "스택 오버플로우(stack overflow)가 발생했다" 라고 표현한다.
