...
# # World Space 예제
    ● Screen Space - Camera 모드는 카메라로부터 일정 거리 떨어진 곳에 UI를 표현하는 방식.
    ● 데드 스페이스 시리즈의 3D 인벤토리 같은 것은 캔버스가 씬에서 오브젝트처럼 동작하게 해야 한다.
    ● 이런 경우는 World Space 모드로 구현해야 한다.
    ● Unity에서 제공하는 Standard Assets Example Project를 활용해서 간단하게 만들어보자.
